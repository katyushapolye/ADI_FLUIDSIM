...
// PARALLEL OPENMP WITH CONVECTION AND DIFFUSION
...
// PARALLEL OPENMP WITH JUST DIFFUSION, we might need to add the font here!

void ADI2D::SolveADI_X_U_DIFF_Step_OPENMP(MAC *gridAnt, MAC *gridSol, double time)
{
    int Nx = gridAnt->Nx;
    int Ny = gridAnt->Ny;
    double dh = ADI2D::dh;
...
#pragma omp for
        for (int i = 1; i < Ny - 1; i++)
        {
            local_U_X_Matrix = local_U_X_DIFF_Matrix;
            int c = 0;
            
            // Build RHS with proper boundary handling
            for (int j = 2; j < Nx - 1; j++)
            {
                // Check if this U face is valid (matches implicit solver logic)
                bool left_fluid = (gridSol->GetSolid(i, j-1) == FLUID_CELL);
                bool right_fluid = (gridSol->GetSolid(i, j) == FLUID_CELL);
                
                // If face is not between at least one fluid cell, set to zero
                if (!left_fluid && !right_fluid)
                {
                    local_U_X_Font(c) = 0.0;
                    local_U_X_Matrix(c, c) = 1.0;
                    if (c >= 1) local_U_X_Matrix(c, c - 1) = 0.0;
                    if (c < local_U_X_Matrix.cols() - 1) local_U_X_Matrix(c, c + 1) = 0.0;
                    c++;
                    continue;
                }
                ...
                // Build Y-direction diffusion term (this sweep handles Y-diffusion explicitly)
                // Match the implicit solver's stencil logic
                double Y_DIFFUSION_TERM = 0.0;
                int y_stencil_count = 0;
                
                // Check top neighbor (i+1)
                bool top_contributes = (gridSol->GetSolid(i+1, j) == FLUID_CELL || 
                                       gridSol->GetSolid(i+1, j-1) == FLUID_CELL);
                // Check bottom neighbor (i-1)
                bool bottom_contributes = (gridSol->GetSolid(i-1, j) == FLUID_CELL || 
                                          gridSol->GetSolid(i-1, j-1) == FLUID_CELL);
                
                if (top_contributes) {
                    Y_DIFFUSION_TERM += gridAnt->GetU(i + 1, j);
                    y_stencil_count++;
                }
                if (bottom_contributes) {
                    Y_DIFFUSION_TERM += gridAnt->GetU(i - 1, j);
                    y_stencil_count++;
                }
                
                // Center contribution (negative)
                Y_DIFFUSION_TERM -= y_stencil_count * gridAnt->GetU(i, j);
                
                local_U_X_Font(c) = gridAnt->GetU(i, j) + sig * Y_DIFFUSION_TERM - (dt / 2.0) * f_ijk;
                
                // Now handle X-direction matrix modifications for this sweep
                // Check solid boundaries
                bool left_solid = (gridSol->GetSolid(i, j-1) == SOLID_CELL);
                bool right_solid = (gridSol->GetSolid(i, j) == SOLID_CELL);
                
                if (left_solid || right_solid) {
                    // Dirichlet BC: u = 0 at solid
                    local_U_X_Font(c) = 0.0;
                    local_U_X_Matrix(c, c) = 1.0;
                    if (c >= 1) {
                        local_U_X_Matrix(c, c - 1) = 0.0;
                    ...
                    if (c < local_U_X_Matrix.cols() - 1) {
                        local_U_X_Matrix(c, c + 1) = 0.0;
                        local_U_X_Matrix(c + 1, c) = 0.0;
                    }
                } else {
                    // Check X-direction neighbors for Neumann BCs
                    // Right neighbor: check cell (i, j)
                    bool right_valid = (gridSol->GetSolid(i, j) == FLUID_CELL);
                    // Left neighbor: check cell (i, j-1)
                    bool left_valid = (gridSol->GetSolid(i, j-1) == FLUID_CELL);
                    
                    // If right cell is not fluid (empty/solid), apply Neumann BC: du/dx = 0
                    if (!right_valid && c < local_U_X_Matrix.cols() - 1) {
                        // Remove coupling to right, adjust diagonal
                        local_U_X_Matrix(c, c + 1) = 0.0;
                        local_U_X_Matrix(c, c) += sig; // Compensate for removed coupling
                    }
                    
                    // If left cell is not fluid (empty/solid), apply Neumann BC: du/dx = 0
                    if (!left_valid && c >= 1) {
                        // Remove coupling to left, adjust diagonal
                        local_U_X_Matrix(c, c - 1) = 0.0;
                        local_U_X_Matrix(c, c) += sig; // Compensate for removed coupling
                    }
                }
            ...
            // Border terms (only if boundary cells are valid)
            local_U_X_Font(0) += sig * gridSol->GetU(i, 1);
            local_U_X_Font(c - 1) += sig * gridSol->GetU(i, Nx - 1);
...
void ADI2D::SolveADI_Y_U_DIFF_Step_OPENMP(MAC *gridAnt, MAC *gridSol, double time)
{
    int Nx = gridSol->Nx;
    int Ny = gridSol->Ny;
    double dh = ADI2D::dh;
...
#pragma omp for
        for (int j = 2; j < Nx - 1; j++)
        {
            local_U_Y_Matrix = local_U_Y_DIFF_Matrix;
            int c = 0;
            
            // Build RHS with proper boundary handling
            for (int i = 1; i < Ny - 1; i++)
            {
                // Check if this U face is valid
                bool left_fluid = (gridSol->GetSolid(i, j-1) == FLUID_CELL);
                bool right_fluid = (gridSol->GetSolid(i, j) == FLUID_CELL);
                ...
                if (!left_fluid && !right_fluid)
                {
                    local_U_Y_Font(c) = 0.0;
                    local_U_Y_Matrix(c, c) = 1.0;
                    if (c >= 1) local_U_Y_Matrix(c, c - 1) = 0.0;
                    if (c < local_U_Y_Matrix.cols() - 1) local_U_Y_Matrix(c, c + 1) = 0.0;
                    c++;
                    continue;
                }
                ...
                // Build X-direction diffusion term (this sweep handles X-diffusion explicitly)
                double X_DIFFUSION_TERM = 0.0;
                int x_stencil_count = 0;
                
                // Right neighbor contributes if right cell is fluid
                bool right_contributes = (gridSol->GetSolid(i, j) == FLUID_CELL);
                // Left neighbor contributes if left cell is fluid
                bool left_contributes = (gridSol->GetSolid(i, j-1) == FLUID_CELL);
                
                if (right_contributes) {
                    X_DIFFUSION_TERM += gridAnt->GetU(i, j + 1);
                    x_stencil_count++;
                }
                if (left_contributes) {
                    X_DIFFUSION_TERM += gridAnt->GetU(i, j - 1);
                ...
                // Center contribution (negative)
                X_DIFFUSION_TERM -= x_stencil_count * gridAnt->GetU(i, j);
                
                local_U_Y_Font(c) = gridAnt->GetU(i, j) + sig * X_DIFFUSION_TERM - (dt / 2.0) * f_ijk;
                
                // Now handle Y-direction matrix modifications for this sweep
                bool left_solid = (gridSol->GetSolid(i, j-1) == SOLID_CELL);
                bool right_solid = (gridSol->GetSolid(i, j) == SOLID_CELL);
                ...
                if (left_solid || right_solid) {
                    ...
                    if (c < local_U_Y_Matrix.cols() - 1) {
                        local_U_Y_Matrix(c, c + 1) = 0.0;
                        local_U_Y_Matrix(c + 1, c) = 0.0;
                    }
                } else {
                    // Check Y-direction neighbors for Neumann BCs
                    // Top neighbor: both cells adjacent to face (i+1,j) must be checked
                    bool top_valid = (gridSol->GetSolid(i+1, j) == FLUID_CELL || 
                                     gridSol->GetSolid(i+1, j-1) == FLUID_CELL);
                    // Bottom neighbor: both cells adjacent to face (i-1,j) must be checked
                    bool bottom_valid = (gridSol->GetSolid(i-1, j) == FLUID_CELL || 
                                        gridSol->GetSolid(i-1, j-1) == FLUID_CELL);
                    
                    // If top neighbor is not valid (empty/solid), apply Neumann BC
                    if (!top_valid && c < local_U_Y_Matrix.cols() - 1) {
                        local_U_Y_Matrix(c, c + 1) = 0.0;
                        local_U_Y_Matrix(c, c) += sig;
                    }
                    
                    // If bottom neighbor is not valid (empty/solid), apply Neumann BC
                    if (!bottom_valid && c >= 1) {
                        local_U_Y_Matrix(c, c - 1) = 0.0;
                        local_U_Y_Matrix(c, c) += sig;
                    }
            ...
            // Border terms
            local_U_Y_Font(0) += sig * gridSol->GetU(0, j);
            local_U_Y_Font(c - 1) += sig * gridSol->GetU(Ny - 1, j);
...
void ADI2D::SolveADI_X_V_DIFF_Step_OPENMP(MAC *gridAnt, MAC *gridSol, double time)
{
    int Nx = gridSol->Nx;
    int Ny = gridSol->Ny;
    double dh = ADI2D::dh;
...
#pragma omp for
        ...
        {
            local_V_X_Matrix = local_V_X_DIFF_Matrix;
            int c = 0;
            
            // Build RHS with proper boundary handling
            for (int j = 1; j < Nx - 1; j++)
            {
                // Check if this V face is valid (matches implicit solver logic)
                bool bottom_fluid = (gridSol->GetSolid(i-1, j) == FLUID_CELL);
                bool top_fluid = (gridSol->GetSolid(i, j) == FLUID_CELL);
                ...
                if (!bottom_fluid && !top_fluid)
                {
                    local_V_X_Font(c) = 0.0;
                    local_V_X_Matrix(c, c) = 1.0;
                    if (c >= 1) local_V_X_Matrix(c, c - 1) = 0.0;
                    if (c < local_V_X_Matrix.cols() - 1) local_V_X_Matrix(c, c + 1) = 0.0;
                    c++;
                    continue;
                }
                ...
                // Top neighbor contributes if top cell is fluid
                bool top_contributes = (gridSol->GetSolid(i, j) == FLUID_CELL);
                // Bottom neighbor contributes if bottom cell is fluid
                bool bottom_contributes = (gridSol->GetSolid(i-1, j) == FLUID_CELL);
                
                if (top_contributes) {
                    Y_DIFFUSION_TERM += gridAnt->GetV(i + 1, j);
                    y_stencil_count++;
                }
                if (bottom_contributes) {
                    Y_DIFFUSION_TERM += gridAnt->GetV(i - 1, j);
                ...
                // Center contribution (negative)
                Y_DIFFUSION_TERM -= y_stencil_count * gridAnt->GetV(i, j);
                
                local_V_X_Font(c) = gridAnt->GetV(i, j) + sig * Y_DIFFUSION_TERM - (dt / 2.0) * f_ijk;
                
                // Now handle X-direction matrix modifications for this sweep
                bool bottom_solid = (gridSol->GetSolid(i-1, j) == SOLID_CELL);
                bool top_solid = (gridSol->GetSolid(i, j) == SOLID_CELL);
                
                if (bottom_solid || top_solid) {
                    // Dirichlet BC: v = 0 at solid
                    local_V_X_Font(c) = 0.0;
                    local_V_X_Matrix(c, c) = 1.0;
                    if (c >= 1) {
                        local_V_X_Matrix(c, c - 1) = 0.0;
                    ...
                    if (c < local_V_X_Matrix.cols() - 1) {
                        local_V_X_Matrix(c, c + 1) = 0.0;
                        local_V_X_Matrix(c + 1, c) = 0.0;
                    }
                } else {
                    // Check X-direction neighbors for Neumann BCs
                    // Right neighbor: both cells adjacent to V-face at (i,j+1)
                    bool right_valid = (gridSol->GetSolid(i-1, j+1) == FLUID_CELL || 
                                       gridSol->GetSolid(i, j+1) == FLUID_CELL);
                    // Left neighbor: both cells adjacent to V-face at (i,j-1)
                    bool left_valid = (gridSol->GetSolid(i-1, j-1) == FLUID_CELL || 
                                      gridSol->GetSolid(i, j-1) == FLUID_CELL);
                    
                    // If right neighbor is not valid (empty/solid), apply Neumann BC
                    if (!right_valid && c < local_V_X_Matrix.cols() - 1) {
                        local_V_X_Matrix(c, c + 1) = 0.0;
                        local_V_X_Matrix(c, c) += sig;
                    }
                    
                    // If left neighbor is not valid (empty/solid), apply Neumann BC
                    if (!left_valid && c >= 1) {
                        local_V_X_Matrix(c, c - 1) = 0.0;
                        local_V_X_Matrix(c, c) += sig;
                    }
            ...
            // Border terms
            local_V_X_Font(0) += sig * gridSol->GetV(i, 0);
            local_V_X_Font(c - 1) += sig * gridSol->GetV(i, Nx - 1);
...
void ADI2D::SolveADI_Y_V_DIFF_Step_OPENMP(MAC *gridAnt, MAC *gridSol, double time)
{
    int Nx = gridSol->Nx;
    int Ny = gridSol->Ny;
    double dh = ADI2D::dh;
...
#pragma omp for
        for (int j = 1; j < Nx - 1; j++)
        {
            local_V_Y_Matrix = local_V_Y_DIFF_Matrix;
            int c = 0;
            
            // Build RHS with proper boundary handling
            for (int i = 2; i < Ny - 1; i++)
            {
                // Check if this V face is valid
                bool bottom_fluid = (gridSol->GetSolid(i-1, j) == FLUID_CELL);
                bool top_fluid = (gridSol->GetSolid(i, j) == FLUID_CELL);
                ...
                if (!bottom_fluid && !top_fluid)
                {
                    local_V_Y_Font(c) = 0.0;
                    local_V_Y_Matrix(c, c) = 1.0;
                    if (c >= 1) local_V_Y_Matrix(c, c - 1) = 0.0;
                    if (c < local_V_Y_Matrix.cols() - 1) local_V_Y_Matrix(c, c + 1) = 0.0;
                    c++;
                    continue;
                }
                ...
                // Right neighbor: check if either cell adjacent to V-face at (i,j+1) is fluid
                bool right_contributes = (gridSol->GetSolid(i-1, j+1) == FLUID_CELL || 
                                         gridSol->GetSolid(i, j+1) == FLUID_CELL);
                // Left neighbor: check if either cell adjacent to V-face at (i,j-1) is fluid
                bool left_contributes = (gridSol->GetSolid(i-1, j-1) == FLUID_CELL || 
                                        gridSol->GetSolid(i, j-1) == FLUID_CELL);
                
                if (right_contributes) {
                    X_DIFFUSION_TERM += gridAnt->GetV(i, j + 1);
                    x_stencil_count++;
                }
                if (left_contributes) {
                    X_DIFFUSION_TERM += gridAnt->GetV(i, j - 1);
                ...
                // Center contribution (negative)
                X_DIFFUSION_TERM -= x_stencil_count * gridAnt->GetV(i, j);
                
                local_V_Y_Font(c) = gridAnt->GetV(i, j) + sig * X_DIFFUSION_TERM - (dt / 2.0) * f_ijk;
                ...
                if (bottom_solid || top_solid) {
                    ...
                    if (c < local_V_Y_Matrix.cols() - 1) {
                        local_V_Y_Matrix(c, c + 1) = 0.0;
                        local_V_Y_Matrix(c + 1, c) = 0.0;
                    }
                } else {
                    // Check Y-direction neighbors for Neumann BCs
                    // Top neighbor: check cell at (i, j)
                    bool top_valid = (gridSol->GetSolid(i, j) == FLUID_CELL);
                    // Bottom neighbor: check cell at (i-1, j)
                    bool bottom_valid = (gridSol->GetSolid(i-1, j) == FLUID_CELL);
                    
                    // If top cell is not fluid (empty/solid), apply Neumann BC: dv/dy = 0
                    if (!top_valid && c < local_V_Y_Matrix.cols() - 1) {
                        local_V_Y_Matrix(c, c + 1) = 0.0;
                        local_V_Y_Matrix(c, c) += sig;
                    }
                    
                    // If bottom cell is not fluid (empty/solid), apply Neumann BC: dv/dy = 0
                    if (!bottom_valid && c >= 1) {
                        local_V_Y_Matrix(c, c - 1) = 0.0;
                        local_V_Y_Matrix(c, c) += sig;
                    }
            ...
            // Border terms
            local_V_Y_Font(0) += sig * gridSol->GetV(1, j);
            local_V_Y_Font(c - 1) += sig * gridSol->GetV(Ny - 1, j);
...
void ADI2D::SolveADIStep(MAC *gridAnt, MAC *gridSol, double time)
{
    ...
    if (SIMULATION.level == LevelConfiguration::STEP || SIMULATION.level == LevelConfiguration::OBSTACLE)
        ADI2D::X_STEP_SOL.SetNeumannBorder();
    ...
    if (SIMULATION.level == LevelConfiguration::STEP || SIMULATION.level == LevelConfiguration::OBSTACLE)
        ADI2D::Y_STEP_SOL.SetNeumannBorder();
...
//not working ebcause the solver is not properly aware of neumman boundary conditions at fluid-empty interfaces
void ADI2D::SolveADIDiffusionStep(MAC *gridAnt, MAC *gridSol, double time)
{
    ...
    X_STEP_SOL.CopySolidMask(*gridAnt);
    Y_STEP_SOL.CopySolidMask(*gridAnt);
    time += SIMULATION.dt / 2.0;
    //ADI2D::X_STEP_SOL.SetBorder(ADI2D::VelocityBorderFunction, ADI2D::PressureFunction, time);

    SolveADI_X_U_DIFF_Step_OPENMP(gridAnt, &X_STEP_SOL,time);

    SolveADI_X_V_DIFF_Step_OPENMP(gridAnt, &X_STEP_SOL,  time);

    time += SIMULATION.dt / 2.0;
    //ADI2D::Y_STEP_SOL.SetBorder(ADI2D::VelocityBorderFunction, ADI2D::PressureFunction, time);

    SolveADI_Y_U_DIFF_Step_OPENMP(&X_STEP_SOL, &Y_STEP_SOL, time);

    SolveADI_Y_V_DIFF_Step_OPENMP(&X_STEP_SOL, &Y_STEP_SOL,  time);
    ...
    gridSol->CopyGrid(Y_STEP_SOL);
    //gridSol->SetBorder(ADI2D::VelocityBorderFunction, ADI2D::PressureFunction, time);
}
...
void ADI2D::SolveADIDiffusionStep(MAC* gridAnt,MAC* gridSol,double time){
    ...
    SolveADI_X_U_DIFF_Step_OPENMP(gridAnt,&X_STEP_SOL,gridAnt,time);
    SolveADI_X_V_DIFF_Step_OPENMP(gridAnt,&X_STEP_SOL,gridAnt,time);
    ...
    SolveADI_Y_U_DIFF_Step_OPENMP(&X_STEP_SOL,&Y_STEP_SOL,gridAnt,time);
    SolveADI_Y_V_DIFF_Step_OPENMP(&X_STEP_SOL,&Y_STEP_SOL,gridAnt,time);
